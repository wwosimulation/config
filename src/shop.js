...
module.exports.items = [
  ...
  { id: "description", name: "Description", price: 25, currency: "rose", description: "Customize your profile description!", response: "You can change your description with `+use description <desc...>`" },
  ...
  { id: "emoji", name: "Emoji", price: 450, currency: "coin", description: "Gain access to `+write` command. Note: The `+emoji` and `+emojilist` commands are in progress and will be fixed as soon as possible.", role: "663389088354664477" },
  ...
  { id: "auto", name: "Auto Reaction", price: 5, currency: "gem", description: "Get an auto reaction whenever someone pings you!", response: "Ask a staff member to get it at an appropriate place/channel. But before asking keep in mind that Carl-bot must be in the same server with the emoji."}
]
...
for (let shopitem in module.exports.items) {
  ...
  embeds[embeds.length - 1].fields.push({name:`${item.name} - ${item.price} ${emojis[item.currency]}`, value: item.description})
}
for (let [i, embed] of embeds.entries()) {
  embed.footer = {text: `Page ${i + 1}/${embeds.length}`}
  embed.title = "Wolvesville Simulation Store"
